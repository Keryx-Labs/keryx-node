...
/// A missed service assignment: the request's window closed with no accepted response.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
impl ServiceLedger {
    ...
    /// request hashes its accepted AiResponses answer; `draw` resolves `(tier, excluded)` to the
