...
use js_sys::BigInt;
use keryx_consensus_client::Header;
use keryx_consensus_client::HeaderT;
...
impl PoW {
    ...
    pub fn new(header: &HeaderT, timestamp: Option<u64>) -> Result<PoW> {
        ...
        let matrix = Matrix::generate(pre_pow_hash, crate::active_salt_version(header.daa_score));
    ...
    pub fn from_raw(pre_pow_hash: &str, timestamp: u64, target_bits: Option<u32>) -> Result<PoW> {
        ...
        let matrix = Matrix::generate(pre_pow_hash, 1); // from_raw has no daa_score; stratum always uses v1 salt
