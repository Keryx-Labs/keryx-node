...
impl CoinbaseManager {
    ...
    pub fn expected_coinbase_transaction<T: AsRef<[u8]>>(
        ...
        // Tier-reward: blue block hash → subsidy multiplier (bps) for its *miner* cut.
        // Empty / missing entry ⇒ full `TIER_REWARD_BPS_DIVISOR` (no penalty), so this is
        ...
        // it never shifts `red_reward_output_index`. Zero before `tier_reward_activation`.
