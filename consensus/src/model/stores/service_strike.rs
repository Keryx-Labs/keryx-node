...
impl DbServiceStrikeStore {
    ...
    /// Deletes every record with an event daa above `daa`; returns how many.
    pub fn delete_above(&self, daa: u64) -> Result<usize, StoreError> {
        let keys: Vec<StrikeLogKey> = self
            .access
            .iterator()
            .filter_map(|r| r.ok())
            .map(|(k, _)| StrikeLogKey::parse(&k))
            .filter(|(event_daa, _)| *event_daa > daa)
            .map(|(event_daa, miner)| StrikeLogKey::new(event_daa, miner))
            .collect();
