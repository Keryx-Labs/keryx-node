...
impl HeaderStoreReader for DbHeadersStore {
    ...
    fn get_header(&self, hash: Hash) -> Result<Arc<Header>, StoreError> {
        Ok(self
            .headers_access
            .read_with_fallbacks3::<HeaderWithBlockLevelPreSeal, HeaderWithBlockLevelPreH3, HeaderWithBlockLevel2>(self.fallback_prefix.as_ref(), hash)?
            .header)
    }

    fn get_header_with_block_level(&self, hash: Hash) -> Result<HeaderWithBlockLevel, StoreError> {
        self.headers_access.read_with_fallbacks3::<HeaderWithBlockLevelPreSeal, HeaderWithBlockLevelPreH3, HeaderWithBlockLevel2>(self.fallback_prefix.as_ref(), hash)
    }
