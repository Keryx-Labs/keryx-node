    ...
    async fn validate_body_in_context_test() {
        let config = ConfigBuilder::new(MAINNET_PARAMS)
            ...
            .edit_consensus_params(|p| {
                ...
                // The sink_search fix (walking a disqualified block's parents to reach a valid sink
                // below it) is PoM-gated, so exercise the post-PoM path here. Otherwise the legacy
                // pre-PoM skip stalls the virtual processor on this DAG (valid sink below block 3).
                p.pom_activation = ForkActivation::always();
            })
            .build();
        ...
        consensus.validate_and_insert_block(valid_block.to_immutable()).virtual_state_task.await.unwrap();
        {
            ...
            assert_match!(
                consensus.validate_and_insert_block(block.clone().to_immutable()).virtual_state_task.await, Err(RuleError::WrongSubsidy(expected,_)) if expected == 5000000000); // 50_000_000_000 / 10 BPS
        ...
        consensus.validate_and_insert_block(valid_block_child.clone().to_immutable()).virtual_state_task.await.unwrap();
        {
            ...
            block.transactions[0].payload[8..16].copy_from_slice(&(5_u64).to_le_bytes());
            block.header.hash_merkle_root = calc_hash_merkle_root(block.transactions.iter());
            assert_match!(consensus.validate_and_insert_block(block.to_immutable()).virtual_state_task.await, Err(RuleError::WrongSubsidy(expected,_)) if expected == 540000000); // 5_400_000_000 sompi/s / 10 BPS
        }
