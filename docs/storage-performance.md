...
instead of `Count(10_000)`, and RMW batches go through RocksDB `MultiGet`. Hit/miss rates are logged
every 10s by the consensus monitor (`Virtual-index cache: balance …; age …`). Measure on a real HDD
IBD before claiming the ~22 BPS ceiling is gone — cold-start and large UTXO churn can still miss.

**`windowed_production_prefix` still does an uncached `SeekForPrev` per rewarded payout — deliberately.**
A result cache was written for it and held back from this release: `cumulative_at` is the ratio
DENOMINATOR, a consensus input, and the proposed cache invalidated during `WriteBatch` construction
rather than after `db.write` — leaving a window in which a concurrent read can repopulate it from
pre-commit state. Whether that window is reachable was not established either way. Given that a
node-local divergence on exactly this class of derived state caused the 2026-07-30 network incident,
the cache ships only once the invalidation is proven to be commit-ordered (or made so).
